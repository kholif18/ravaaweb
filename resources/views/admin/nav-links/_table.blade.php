@php
    $positionLabels = [
        'navbar' => '<span class="badge" style="background:rgba(34,197,94,0.1);color:#15803d;font-size:0.7rem;">Desktop</span>',
        'mobile' => '<span class="badge" style="background:rgba(59,130,246,0.1);color:#1d4ed8;font-size:0.7rem;">Mobile</span>',
        'both' => '<span class="badge" style="background:rgba(var(--accent-rgb,79,110,247),0.1);color:var(--accent);font-size:0.7rem;">Keduanya</span>',
    ];
@endphp
    ...
    <table class="table align-middle mb-0">
        ...
        <tbody id="sortable-nav-links">
            ...
            <tr data-id="{{ $link->id }}">
                ...
                <td style="font-size:0.82rem;font-weight:600;color:var(--text-primary);">
                    ...
                    @if($link->children->count())
                        <span class="badge" style="background:rgba(var(--accent-rgb,79,110,247),0.1);color:var(--accent);font-size:0.65rem;margin-left:4px;">{{ $link->children->count() }} child</span>
                    @endif
                </td>
                <td style="font-size:0.78rem;color:var(--accent);max-width:250px;word-break:break-all;">
                    ...
                    @else
                        <code style="font-size:0.78rem;">{{ $link->url }}</code>
                    @endif
                </td>
                <td>{!! $positionLabels[$link->position] ?? '-' !!}</td>
                <td>
                    @if($link->is_active)
                ...
                <td class="text-center" style="white-space:nowrap;">
                    <div class="d-flex justify-content-center gap-1">
                        ...
                        @endif
                        <button type="button" class="btn btn-icon btn-sm"
                                onclick="deleteNavLink({{ $link->id }}, '{{ addslashes($link->label) }}')"
                                title="Hapus"
            ...
            <tr data-id="{{ $child->id }}" style="background:rgba(0,0,0,0.015);">
                ...
                <td style="font-size:0.78rem;color:var(--accent);max-width:250px;word-break:break-all;">
                    @if(strpos($child->url, 'http') === 0)
                        <a href="{{ $child->url }}" target="_blank">{{ $child->url }}</a>
                    @else
                        <code style="font-size:0.78rem;">{{ $child->url }}</code>
                    @endif
                </td>
                <td>{!! $positionLabels[$child->position] ?? '-' !!}</td>
                <td>
                    @if($child->is_active)
                        <span class="badge" style="background:rgba(34,197,94,0.1);color:#15803d;font-size:0.7rem;">Aktif</span>
                    @else
                ...
                <td class="text-center" style="white-space:nowrap;">
                    <div class="d-flex justify-content-center gap-1">
                        ...
                        @if($child->is_active)
                        <button type="button" class="btn btn-icon btn-sm"
                                onclick="updateStatus({{ $child->id }}, 0, '{{ addslashes($child->label) }}')"
                                title="Nonaktifkan"
                                style="width:28px;height:28px;border-radius:6px;background:rgba(234,179,8,0.1);color:#a16207;">
                            <i class="bi bi-pause-circle" style="font-size:0.75rem;"></i>
                        </button>
                        @else
                        <button type="button" class="btn btn-icon btn-sm"
                                onclick="updateStatus({{ $child->id }}, 1, '{{ addslashes($child->label) }}')"
                                title="Aktifkan"
                                style="width:28px;height:28px;border-radius:6px;background:rgba(34,197,94,0.1);color:#15803d;">
                            <i class="bi bi-play-circle" style="font-size:0.75rem;"></i>
                        </button>
                        @endif
                        <button type="button" class="btn btn-icon btn-sm"
                                onclick="deleteNavLink({{ $child->id }}, '{{ addslashes($child->label) }}')"
                                title="Hapus"
                                style="width:28px;height:28px;border-radius:6px;background:rgba(239,68,68,0.1);color:#ef4444;">
                            <i class="bi bi-trash" style="font-size:0.75rem;"></i>
                        </button>
...
@if($navLinks->hasPages())
    <div class="d-flex justify-content-center mt-3">
        {{ $navLinks->links('vendor.pagination.custom') }}
    </div>
@endif
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-3">
    ...
    <span style="font-size:0.8rem;color:var(--text-muted);">
        Menampilkan {{ $navLinks->firstItem() }}-{{ $navLinks->lastItem() }} dari {{ $navLinks->total() }} link
    </span>
</div>
